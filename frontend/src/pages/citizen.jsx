...
const AlertCard = ({ alert }) => {
  ...
  return (
    <div className={`p-6 rounded-xl border-l-4 ${style.color} bg-slate-800 mb-6 shadow-lg border border-slate-700`}>
      <div className="flex justify-between">
        <div>
          <h2 className="font-bold uppercase text-white">Active Crisis</h2>
          <p className="text-lg text-slate-200">{style.label}</p>
        </div>
        <div className="text-right text-sm">
          <span className={`px-2 py-1 rounded text-white ${confidence > 80 ? 'bg-green-600' : 'bg-yellow-600'}`}>
            {confidence}% Confidence
          </span>
          <div className="opacity-70 mt-1 flex items-center gap-1 text-slate-400">
            <Clock size={12} />
      ...
      <div className="mt-2 flex items-center gap-2 text-sm opacity-70 text-slate-400">
        <MapPin size={14} />
...
export default function Citizen() {
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100">
      <header className="bg-slate-900 text-white p-4 flex justify-between items-center border-b border-slate-800 shadow-md sticky top-0 z-50">
      ...
      <div className="w-[96%] mx-auto py-8">
        <h1 className="text-2xl font-bold mb-4 text-white">Citizen Dashboard</h1>

        {loading ? <p className="text-slate-400">Loading…</p> :
          alerts.length ? alerts.map(a => <AlertCard key={a.alert_id} alert={a} />) :
            <div className="bg-green-900/20 border border-green-800 p-6 text-center rounded-xl">
              <CheckCircle className="mx-auto mb-2 text-green-500" />
              <span className="text-green-400">No active threats</span>
            </div>
        }

        <div className="bg-slate-800 rounded-xl h-[400px] mt-6 border border-slate-700 overflow-hidden shadow-lg">
          <MapView
