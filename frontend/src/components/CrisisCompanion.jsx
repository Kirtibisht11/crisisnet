...
export default function CrisisCompanion() {
  ...
  return (
    <div className="bg-slate-800 rounded-xl border border-slate-700 shadow-lg flex flex-col h-[700px] max-w-2xl mx-auto">
      {/* HEADER */}
      <div className="p-4 bg-gradient-to-r from-blue-800 to-blue-600 rounded-t-xl text-white">
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className={`p-2 rounded-full ${isHindi ? 'bg-green-500' : 'bg-blue-700'}`}
            >
            ...
            <div className="flex gap-2">
              <button 
                ...
                className={`p-2 rounded-full ${autoSpeak ? 'bg-blue-700' : 'bg-gray-700'}`}
              >
      ...
      {messages.length === 0 ? (
        <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-gradient-to-b from-slate-900 to-slate-800">
          <div className="text-center mb-6">
            <div className="inline-flex items-center justify-center gap-3 mb-3">
              <div className="w-12 h-12 bg-gradient-to-r from-blue-700 to-blue-500 rounded-full flex items-center justify-center">
                <div className="text-white text-2xl">🛟</div>
              </div>
              <div>
                <h2 className="text-2xl font-bold text-white">Crisis Net Support</h2>
                <p className="text-slate-400">Multipurpose emergency assistance</p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {CRISIS_TYPES.map((crisis) => (
              <button
                ...
                className="p-4 bg-slate-800 border-2 border-slate-700 rounded-lg text-left hover:border-blue-700 hover:shadow-lg transition"
              >
          ...
          {/* Voice Instructions */}
          <div className="p-4 bg-gradient-to-r from-blue-900/20 to-cyan-900/20 border border-blue-800 rounded-lg">
        ...
        <>
          {/* CHAT INTERFACE */}
          <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-gradient-to-b from-slate-900 to-slate-800" ref={scrollRef}>
            {messages.map((msg, idx) => (
              <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[85%] p-3 rounded-lg text-sm ${
                  msg.role === 'user' 
                    ? 'bg-blue-700 text-white rounded-br-none' 
                    : 'bg-slate-700 border-2 border-blue-900 text-white rounded-bl-none'
                }`}>
                  {msg.text}
                </div>
              </div>
            ...
            {loading && (
              <div className="flex justify-start">
                <div className="bg-slate-700 border-2 border-blue-900 text-white p-3 rounded-lg rounded-bl-none">
                  <div className="flex items-center gap-2">
                    <div className="w-4 h-4 bg-gradient-to-r from-blue-700 to-blue-500 rounded-full animate-pulse"></div>
          ...
          <div className="p-3 border-t border-slate-700 bg-slate-800 rounded-b-xl">
            <div className="flex gap-2">
              <button
                onClick={toggleListening}
                className={`p-3 rounded-lg ${isListening ? 'bg-blue-700 text-white' : 'bg-slate-700 text-slate-300'}`}
              >
              ...
              <input
                ...
                className="flex-1 px-4 py-2 bg-slate-900 border border-slate-600 text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-700 placeholder-slate-500"
              />
